...
//             nf.setMaximumFractionDigits(2);
			...
			String folderPath = System.getProperty("user.home") + "/Downloads/" + Constaints.DOWNLOAD_FOLDER_NAME
					+ "/Bill/";
...
//            PdfWriter writer = new PdfWriter(new FileOutputStream(fileName + ".pdf"));
			...
			/* ----------- HEADER ----------- */
			Table header = new Table(UnitValue.createPercentArray(new float[] { 1.5f, 4f, 2f })).useAllAvailableWidth();

			// Logo cell (try catch to avoid failure)
			try {
				Image logo = new Image(
						com.itextpdf.io.image.ImageDataFactory.create("C:\\Users\\Ramesh\\Downloads\\logo.jpg"));
			...
			Paragraph cname = new Paragraph(
					user.getCompanyName() == null ? "RAHUL MOBILE GALLERY" : user.getCompanyName()).setFont(bold)
					.setFontSize(9).setTextAlignment(TextAlignment.CENTER);

			String addressStr = (user.getAddressLine1() == null ? "1743, Dhuri Line Rd, Abdullapur Basti, Azad Nagar,"
					: user.getAddressLine1()) + "\n" + (user.getCity() == null ? "Mohali" : user.getCity()) + ", "
					+ (user.getState() == null ? "Punjab" : user.getState()) + " - "
					+ (user.getPincode() == null ? "160055" : user.getPincode());

			Paragraph addr = new Paragraph(addressStr).setFont(normal).setFontSize(6)
					.setTextAlignment(TextAlignment.CENTER);

			Paragraph phone = new Paragraph(
					"Mob. : " + (user.getMobile_no() == null ? "9876543210" : user.getMobile_no())).setFont(normal)
					.setFontSize(6).setTextAlignment(TextAlignment.CENTER);

			Cell companyCell = new Cell().setBorder(Border.NO_BORDER).add(cname).add(addr).add(phone);

			header.addCell(companyCell);

			// Slip + Barcode cell
			Paragraph slipTitle = new Paragraph("E BILL").setFont(bold).setFontSize(7)
					.setTextAlignment(TextAlignment.CENTER);

			Barcode128 barcode = new Barcode128(pdf);
			barcode.setCode(buyer.getInvoiceNumber());

			Image barcodeImage = new Image(barcode.createFormXObject(null, null, pdf)).setAutoScale(true)
					.setHorizontalAlignment(HorizontalAlignment.CENTER);

			Cell slipCell = new Cell().setBorder(Border.NO_BORDER).add(slipTitle).add(barcodeImage);
			...
			invoiceTable.addCell(new Cell()
					.add(new Paragraph("Invoice Number: " + buyer.getInvoiceNumber()).setFont(bold).setFontSize(6))
					.setBorder(Border.NO_BORDER));
			invoiceTable.addCell(
					new Cell().add(new Paragraph("Invoice Date : " + LocalDate.now()).setFont(bold).setFontSize(6))
							.setBorder(Border.NO_BORDER));
			...
			if (buyer.getGstNumber() == null) {
				...
				customerTable.addCell(new Cell().add(new Paragraph(
						"Customer : " + (buyer.getBuyerName() != null ? buyer.getBuyerName() : buyer.getCustumberId()))
						.setFont(normal).setFontSize(6)).setBorder(Border.NO_BORDER));
				customerTable.addCell(new Cell().add(
						new Paragraph("Phone : " + (buyer.getMobileNumber() != null ? buyer.getMobileNumber() : "N/A"))
								.setFont(normal).setFontSize(6))
						.setBorder(Border.NO_BORDER));
				customerTable.addCell(new Cell().add(new Paragraph(
						"Address : " + (buyer.getBuyerAddress() != null ? buyer.getBuyerAddress() : "N/A"))
						.setFont(normal).setFontSize(6)).setBorder(Border.NO_BORDER));
				customerTable.addCell(new Cell()
						.add(new Paragraph("Email'Id : " + (buyer.getEmailId() != null ? buyer.getEmailId() : "N/A")))
						.setFont(normal).setFontSize(6).setBorder(Border.NO_BORDER));
			...
			String shipName = (buyer.getCompanyName() == null || buyer.getCompanyName().trim().isEmpty())
					? "RS Communication Pvt Ltd"
					: buyer.getCompanyName();
			String shipAddress = (buyer.getCompanyAddress() == null || buyer.getCompanyAddress().trim().isEmpty())
					? "Mohali Punjab"
					: buyer.getCompanyAddress();
			String shipState = (buyer.getState() == null || buyer.getState().trim().isEmpty()) ? "Punjab"
					: buyer.getState();
			String shipDist = (buyer.getDistric() == null || buyer.getDistric().trim().isEmpty()) ? "Mohali"
					: buyer.getDistric();

			String shipPincode = (buyer.getPinCode() == null || buyer.getPinCode().trim().isEmpty()) ? "160055"
					: buyer.getPinCode();
			...
			boolean isIgstNotApplicable = buyer.getState() == null
					|| buyer.getState().equalsIgnoreCase(user.getState());
			...
			itemTable.addHeaderCell(new Cell().add(new Paragraph("SR").setFont(bold).setFontSize(6))
					.setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
			itemTable.addHeaderCell(new Cell().add(new Paragraph("ITEM").setFont(bold).setFontSize(6))
					.setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
			...
			if (isGstApplicable) {
				// Create header row
				if (isIgstNotApplicable) {
					itemTable.addHeaderCell(new Cell().add(new Paragraph("CGST").setFont(bold).setFontSize(6))
							.setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));

					itemTable.addHeaderCell(new Cell().add(new Paragraph("SGST").setFont(bold).setFontSize(6))
							.setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
				} else {
					itemTable.addHeaderCell(new Cell().add(new Paragraph("IGST").setFont(bold).setFontSize(6))
							.setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
				}
			}
...
//            itemTable.addHeaderCell(new Cell().add(new Paragraph(buyer.getState()!=null?(buyer.getState().equalsIgnoreCase(user.getState())?"SGST+CGST":"IGST"):"SGST+CGST").setFont(bold).setFontSize(6)).setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
			itemTable.addHeaderCell(new Cell().add(new Paragraph("AMOUNT").setFont(bold).setFontSize(6))
					.setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
			...
			for (int i = 0; i < sales.size(); i++) {
				...
				BigDecimal basePrice = isGstApplicable ? BigDecimal.valueOf(s.getSoldPrice()).multiply(BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(100).add(gstRate), 8, java.math.RoundingMode.HALF_UP) 
				...
				itemTable.addCell(new Cell().add(new Paragraph(String.valueOf(i + 1))).setFont(normal).setFontSize(6)
						.setTextAlignment(TextAlignment.CENTER));
...
//				itemTable.addCell(new Cell()
//						.add(new Paragraph(
//								nf.format(isGstApplicable ? basePrice : new BigDecimal(s.getSoldPrice())).toString()))
//						.setFont(normal).setFontSize(6).setTextAlignment(TextAlignment.RIGHT));
				...
				if (isGstApplicable) {
					...
					if (isIgstNotApplicable) {// buyer.getState() == null) {
						...
						itemTable.addCell(new Cell().add(
								new Paragraph(sgstValue + "").setFontSize(6).setTextAlignment(TextAlignment.CENTER)));
					} else {
				...
				itemTable.addCell(new Cell().add(new Paragraph(nf.format(amt).toString())).setFont(normal)
						.setFontSize(6).setTextAlignment(TextAlignment.RIGHT));
			}
			...
			/* ---------------- PRICE SUMMARY ---------------- */
			double gst = sales.stream().mapToDouble(s -> {
...
//				}
//
//				double rate = isGstApplicable ? (s.getSoldPrice() / (1 + Integer.valueOf(gstValue2.getIgst()) / 100.0))
//						: s.getSoldPrice();
...
//			double grandTotal = subTotal -discount+ (isGstApplicable ? gst : 0.0);// - discount;
			...
			Paragraph notes = new Paragraph().add(new Text("Notes : ").setFont(bold).setFontSize(7)).add(new Text(
					buyer.getNoteComment() == null || buyer.getNoteComment().isEmpty() ? "" : buyer.getNoteComment())
					.setFontSize(6));
			...
			Paragraph warranty = new Paragraph().setFontSize(6)
					.add(new Text("Warranty & Return Policy\n").setFont(bold)).add("• Warranty as per company policy\n")
...
//            addLine(document);
	...
	private static Cell cell(String text, PdfFont font) {
		...
		return new Cell().add(new Paragraph(text).setFont(font).setFontSize(6)).setBorder(Border.NO_BORDER)
				.setTextAlignment(TextAlignment.RIGHT);
	}
}
